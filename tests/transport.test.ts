...
describe('Factory Method Pattern - 物流运输示例', () => {
  ...
  describe('物流公司（创建者）', () => {
    it('RoadLogistics 应该创建 Truck 并规划陆地配送', () => {
      ...
      expect(result).toBe(
        'Logistics: planning delivery -> Truck: deliver by land in boxes.'
      )
    })
    ...
    it('SeaLogistics 应该创建 Ship 并规划海运配送', () => {
      ...
      expect(result).toBe(
        'Logistics: planning delivery -> Ship: deliver by sea in containers.'
      )
    })
  ...
  describe('多态性和扩展性', () => {
    ...
    it('应该支持扩展新的运输方式和物流公司', () => {
      ...
      expect(result).toBe(
        'Logistics: planning delivery -> Plane: deliver by air in cargo holds.'
      )
    })
  })
